...
#
# File: profile-workspace.controller.spec.coffee
###
...
describe "ProfileWorkspace", ->
    ...
    it "load paginated items", (done) ->
        $scope = $rootScope.$new()
        ctrl = $controller("ProfileWorkspace", $scope, {user: user})
        ...
        mocks.userServices.getWorkspace.withArgs(user.get("id"), 1, null, null).promise().resolve(items1)
        mocks.userServices.getWorkspace.withArgs(user.get("id"), 2, null, null).promise().resolve(items2)
    ...
    it "filter items by text query", (done) ->
        ...
        mocks.userServices.getWorkspace.withArgs(user.get("id"), 1, null, textQuery).promise().resolve(items)
    ...
    it "show only items of projects", (done) ->
        ...
        mocks.userServices.getWorkspace.withArgs(user.get("id"), 1, type, null).promise().resolve(items)
    ...
    it "shou loading spinner during the call to the api", (done) ->
        ...
        mockPromise = mocks.userServices.getWorkspace.withArgs(user.get("id"), 1, null, null).promise()
    ...
    it "shou no results placeholder", (done) ->
        ...
        mocks.userServices.getWorkspace.withArgs(user.get("id"), 1, null, null).promise().resolve(items)
